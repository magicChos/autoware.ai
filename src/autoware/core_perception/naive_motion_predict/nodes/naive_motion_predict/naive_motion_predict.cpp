...
/**
 * @brief 初始化ROS marker保存到predicted_line
 * 
 * @param [in] header 
 * @param [in] position 
 * @param [in] object_id 
 * @param [out] predicted_line 
 */
 ...
 * 
 * @param [in] object 
 * @param [out] predicted_objects_vec 
 * @param [out] predicted_line 
 */
...
bool NaiveMotionPredict::isObjectValid(const autoware_msgs::DetectedObject &in_object)
{
    ...
    if (!in_object.valid || std::isnan(in_object.pose.orientation.x) || std::isnan(in_object.pose.orientation.y) || std::isnan(in_object.pose.orientation.z) || std::isnan(in_object.pose.orientation.w) || std::isnan(in_object.pose.position.x) || std::isnan(in_object.pose.position.y) || std::isnan(in_object.pose.position.z) || (distance <= filter_out_close_object_threshold_) || (in_object.dimensions.x <= 0) || (in_object.dimensions.y <= 0) || (in_object.dimensions.z <= 0))
    {
        return false;
    }
    return true;
