                          ...
                          const int in_image_width)
{
    ...
    for (unsigned int ctx_idx = 0; ctx_idx < lane_id_vector.size(); ctx_idx++)
    {
        ...
        for (lamp_iterator = signals_lamps.begin(); lamp_iterator < signals_lamps.end(); lamp_iterator++)
        {
            ...
            if (lamp_iterator->linkId == lane_id_vector.at(ctx_idx) && 0 < img_x - radius - 1.5 * radius && img_x + radius + 1.5 * radius < in_image_width && 0 < img_y - radius - 1.5 * radius && img_y + radius + 1.5 * radius < in_image_height)
            {
        ...
        {
            if (current_signal_context.signalID == out_signal_contexts->at(i).signalID && current_signal_context.lampRadius != INT_MAX)
            {
