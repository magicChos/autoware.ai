 ...
 * 
 * @param [in] in_point 
 * @param [in] in_transform 
 * @return cv::Point3f 经过tf变换后的点
 */
cv::Point3f ROSRangeVisionFusionApp::TransformPoint(const geometry_msgs::Point &in_point,
...
/**
 * @brief 利用相机内参将相机坐标系下的三维坐标变换到图像平面坐标系的像素坐标
 * 
 * @param [in] in_point 摄像机坐标系下的三维坐标
 * @return cv::Point2i 像平面坐标系下的二维坐标
 */
...
/**
 * @brief lidar坐标系下的点经过tf变换投影到图像平面上，判定该投影点是否在图像平面范围内
 * 
 * @param [in] in_detection 
 * @return true 
 * @return false 
 */
bool ROSRangeVisionFusionApp::IsObjectInImage(const autoware_msgs::DetectedObject &in_detection)
{
    // 将Lidar坐标系下的点变换到相机坐标系下
...
/**
 * @brief 计算检测物体的3d box，将3dbox的八个顶点投影的图像平面上，返回八个投影点组成的boudingbox
 * 
 * @param [in] in_detection 
 * @return cv::Rect 返回八个投影点组成的boudingbox
 */
...
/**
 * @brief 将利用Range检测的结果投影的图像上，分别保存在图像范围内、外的检测物体
 * 
 * @param [in] in_range_detections 
 * @param [out] out_in_cv_range_detections 记录在图像范围内的检测物体
 * @param [out] out_out_cv_range_detections 记录在图像范围外的检测物体
 */
void ROSRangeVisionFusionApp::TransformRangeToVision(
    const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections, autoware_msgs::DetectedObjectArray &out_in_cv_range_detections,
...
void ROSRangeVisionFusionApp::CalculateObjectFeatures(autoware_msgs::DetectedObject &in_out_object, bool in_estimate_pose)
{
...
{
    return sqrt(in_object.dimensions.x * in_object.dimensions.x + in_object.dimensions.y * in_object.dimensions.y + in_object.dimensions.z * in_object.dimensions.z);
}
    ...
    const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections)
{
    ...
    {
        camera_lidar_tf_ = FindTransform(image_frame_id_, in_range_detections->header.frame_id);
    }
...
void ROSRangeVisionFusionApp::IntrinsicsCallback(const sensor_msgs::CameraInfo &in_message)
{
...
void ROSRangeVisionFusionApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
    //get params
    ...
    if (!sync_topics)
    {
        detections_range_subscriber_ = in_private_handle.subscribe(detected_objects_vision, 1, &ROSRangeVisionFusionApp::VisionDetectionsCallback, this);
    ...
    {
        vision_filter_subscriber_ = new message_filters::Subscriber<autoware_msgs::DetectedObjectArray>(node_handle_, detected_objects_vision, 1);
        range_filter_subscriber_ = new message_filters::Subscriber<autoware_msgs::DetectedObjectArray>(node_handle_, detected_objects_range, 1);
